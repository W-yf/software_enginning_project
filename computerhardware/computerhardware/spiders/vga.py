...
class VgaSpider(scrapy.Spider):
    ...
    def third_parse(self, response):  # 浏览器自动添加tbody，不要再xpath中出现tbody
        VGA = items.VGA()
        VGA['VGA_name'] = response.xpath(
            "/html/body/div[@class='product-model page-title clearfix']/h1[@class='product-model__name']/text()").get()
        VGA['VGA_chip'] = response.xpath("//table[1]//tr[3]//td//span//text()").get()
        VGA['VGA_price'] = response.xpath(
            "/html/body/div[@class='wrapper  clearfix']/div[@class='side']/div[@class='goods-card']/div[@class='goods-card__price']/span/text()").get()
        VGA['VGA_series'] = response.xpath("//table[1]//tr[4]//td//span//text()").get()
        VGA['VGA_capacity'] = response.xpath("//table[2]//tr[3]//td//span//text()").get()
        VGA['VGA_brand'] = response.xpath("//table[2]//tr[4]//td//span//text()").get()
        yield VGA  # 返回产品参数